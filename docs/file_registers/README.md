# 📂 File Registers — Übersicht

> Datei-Register der Module — Pfade auf Monorepo-Struktur regeneriert (06.09.2026).
> Das Register des Monorepos selbst wird kanonisch im Code-Repo geführt: `a-townchain-os/FILE_REGISTER.md`.
> Jedes Register listet alle physisch existierenden Dateien mit Zeilenzahl und Typ.

---

## Code-Repos mit eigenem Wiki-Repo

| Code-Repo | Wiki-Repo | Datei-Register |
|-----------|-----------|-----------------|
| atc-kernel | atc-kernel-wiki | [📄](atc-kernel_FILE_REGISTER.md) |
| atcnet | atcnet-wiki | [📄](atcnet_FILE_REGISTER.md) |
| atc-standards | atc-standards-wiki | [📄](atc-standards_FILE_REGISTER.md) |
| atc-contracts | atc-contracts-wiki | [📄](atc-contracts_FILE_REGISTER.md) |
| atc-shivamon | atc-shivamon-wiki | [📄](atc-shivamon_FILE_REGISTER.md) |
| atc-gateway | atc-gateway-wiki | [📄](atc-gateway_FILE_REGISTER.md) |
| atc-ui | atc-ui-wiki | [📄](atc-ui_FILE_REGISTER.md) |
| atclang | atclang-wiki | [📄](atclang_FILE_REGISTER.md) |
| atc-franchise | atc-franchise-wiki | [📄](atc-franchise_FILE_REGISTER.md) |

## Code-Repos ohne eigenes Wiki (nur hier)

| Code-Repo | Datei-Register |
|-----------|-----------------|
| atc-atclang | [📄](atc-atclang_FILE_REGISTER.md) |
| atc-blockchain | [📄](atc-blockchain_FILE_REGISTER.md) |
| atc-backend | [📄](atc-backend_FILE_REGISTER.md) |
| atc-frontend | [📄](atc-frontend_FILE_REGISTER.md) |
| atc-mobile | [📄](atc-mobile_FILE_REGISTER.md) |
| atc-genesis-engine | [📄](atc-genesis-engine_FILE_REGISTER.md) |
| atc-aistudio | [📄](atc-aistudio_FILE_REGISTER.md) |
| atc-shivacore | [📄](atc-shivacore_FILE_REGISTER.md) |
| atc-windows-edition | [📄](atc-windows-edition_FILE_REGISTER.md) |
| atc-linux-edition | [📄](atc-linux-edition_FILE_REGISTER.md) |
| atc-shivacore-tools | [📄](atc-shivacore-tools_FILE_REGISTER.md) |
| atc-atcpkg | [📄](atc-atcpkg_FILE_REGISTER.md) |

---

*Auto-generiert 2026-08-05 · Aurora (MasterBrain · Base44) · v1.0.0*
